...
async def sendmessage_get_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if int(update.effective_user.id) == int(admin_id):
        ...
        await update.message.reply_text(
            "收到w，那么要发送什么内容呢？可以使用 <code>/cancel</code> 取消哦", parse_mode=ParseMode.HTML
        )
        return GETMESSAGE
    else:
        return ConversationHandler.END
...
handlers = [
    ConversationHandler(
        entry_points=[CommandHandler("send", sendmessage_get_id)],
        states={
            GETMESSAGE: [MessageHandler(~filters.Regex("^\/cancel$"), sendmessage)]
        },

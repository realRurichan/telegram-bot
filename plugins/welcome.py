            ...
            if user.status == "administrator" or user.status == "creator":
                ...
                if int(res.fetchone()[0]) == 0:
                    try:
                        ...
                        sql_command = "INSERT INTO WELCOME (GROUP_ID,REPLY) VALUES (" + group_id + ",?);"
                    ...
                    try:
                        match_context = update.message.text[12: ]
                        sql_command = "UPDATE WELCOME SET REPLY = ? WHERE GROUP_ID = \"" + group_id + "\";"
                        cur.execute(sql_command, (match_context, ))
                        con.commit()
                        logger.debug("SQL command executed: " + sql_command)
                        text = "欢迎语设置成功w"
...
'''
handlers = [CommandHandler("welcomeset", welcomeset), ChatMemberHandler(welcome, ChatMemberHandler.ANY_CHAT_MEMBER)]

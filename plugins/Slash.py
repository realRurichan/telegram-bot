...
class User:
    ...
    def __get_user_by_username(self):
        ...
        self.name = re.search(
            r'(?<=<meta property="og:title" content=").*(?=")', r.text, re.IGNORECASE
        ).group(0)
    ...
    def mention(self, mention_self: bool = False, pure: bool = False) -> str:
        ...
        mention_deep_link = (
            f"tg://resolve?domain={self.username}"
    ...
    def __eq__(self, other):
        return type(self) == type(other) and (
            ((self.uid or other.uid) and self.uid == other.uid)
            or ((self.username or other.username) and self.username == other.username)
        )
...
async def parse_command(ctx: CallbackContext) -> Optional[dict[str, Union[str, bool]]]:
    ...
    result = {
        "predicate": htmlEscape(predicate),
...
def get_text(user_from: User, user_rpl: User, command: dict):
    ...
    if predicate == "":
        ...
        ret += get_tail(
            (complement or user_rpl.mention(mention_self=rpl_self, pure=True))[-1]
        )
    elif complement:
    ...
    if from_user == rpl_user:
        ...
        if mention_match:
            ...
            if mention_match:
                ...
                complement = (
                    complement[: mention_match.start()]
                    + complement[mention_match.end() :]
                )
...
handlers = [
    MessageHandler(
        Filters.Regex(parser) & ~Filters.UpdateType.EDITED & ~filters.COMMAND, reply
    )
]

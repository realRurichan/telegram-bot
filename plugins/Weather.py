...
async def weather(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lon = location['lon']
    lat = location['lat']
    weather_api = requests.get(f'https://devapi.qweather.com/v7/weather/now?key={qweather_key}&location={lon},{lat}&lang=zh')
    ...
    try:
        air_quality_api = requests.get(f'https://devapi.qweather.com/v7/air/now?key={qweather_key}&location={lon},{lat}&lang=zh')

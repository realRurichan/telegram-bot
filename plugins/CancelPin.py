...
async def cancelpin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = cur.execute(
        'select BOOL  from "' + group_id + '" where COMMAND = "CANCELPIN";'
    )
    if int(res.fetchone()[0]) == 1:
        await asyncio.sleep(30)
        await context.bot.get_me()
        result = await context.bot.unpin_chat_message(
            update.effective_message.chat.id, update.effective_message.message_id
        )
        if result == True:
